...
from balloon_finder_rc1 import BalloonFinder
    ...
    def update_vector(self,bf): 
        for i in range(20):
            ...
            if len(balloon_list)>0: # and balloon_list != None:
                ...
                if all( i is not None for i in true_balloon):
                    continue
                ...
                self.vehicle.mode = VehicleMode("GUIDED")
                time.sleep(2)
                print "Flying to way balloon point"
		time.sleep(2)
                self.goto_position_target_local_ned(x,y,z-30)
                #time.sleep(10)
                vec = [x,y,z]
                rotated_angle=0
                return vec
...
#                    rotated_angle=0
 ...
 #               time.sleep(.033)
    ...
    def run(self):
        ...
        self.take_off(2)
        time.sleep(2)
	#home_location = LocationGlobal(self.vehicle.location.global_frame.lat,self.vehicle.location.global_frame.lon,2)
        home_location = LocationGlobalRelative(self.vehicle.location.global_frame.lat,self.vehicle.location.global_frame.lon,2)
        print "Home set as: {}".format(home_location)
        #time.sleep(3) 
        #self.fly_to_waypoint(self.waypoints[current_waypoint])
        #self.vehicle.mode = VehicleMode("AUTO")
        #time.sleep(4)
        print "Finding Balloon"
	time.sleep(3)
